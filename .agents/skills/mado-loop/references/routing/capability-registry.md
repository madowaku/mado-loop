...
| OpenRouter worker | Any bounded reasoning/coding subtask | Optional external worker hub | Only when configured and selected by the orchestrator | Optional `SKIPPED`; never invent credentials or silently change model/data policy |
| NVIDIA NIM hosted worker | Public bounded reasoning/coding subtask; private only with explicit opt-in | Optional external OpenAI-compatible worker | Only when `NVIDIA_API_KEY` + `MADO_NVIDIA_MODEL` are configured; private payloads additionally require explicit consent | Optional `SKIPPED`; forbidden for secret payloads; model IDs remain configuration because the hosted catalog changes |
...
Provider capability and model identity are deliberately separate. A capability says what execution lane is permitted; a model ID says which currently configured model serves that lane. Do not encode transient hosted-model catalogs into the capability contract. `provider_router.py plan` may expose a deterministic `fallback_candidates` list, but every candidate must already satisfy the same sensitivity policy before it can appear in that list.
