...
func execute(params: Dictionary) -> void:
    utils_script.log_info("Adding node to scene: " + str(params.get("scene_path", "")))

    var editor = scene_editor_script.new()
    var success = editor.open_existing_scene(params.get("scene_path", ""))
    if success:
        success = editor.add_node(params)
    if success:
        success = editor.save_scene()
    if success:
        utils_script.log_info("Node '%s' of type '%s' added successfully" % [str(params.get("node_name", "")), str(params.get("node_type", ""))])
    editor.cleanup()
