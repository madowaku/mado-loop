class_name GodotSkillReorderNode
extends RefCounted
...
func execute(params: Dictionary) -> void:
    utils_script.log_info("Reordering node in scene: " + str(params.get("scene_path", "")))

    var editor = scene_editor_script.new()
    var success = editor.open_existing_scene(params.get("scene_path", ""))
    if success:
        success = editor.reorder_node(params)
    if success:
        success = editor.save_scene()
    if success:
        utils_script.log_info("Node reordered successfully")
    editor.cleanup()
