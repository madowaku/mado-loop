...
Use `scripts/codex_plus_lane.py` when MADO LOOP is already running under Codex with **Sign in with ChatGPT** and a bounded worker should use the included ChatGPT-plan Codex allowance instead of an API key. Keep Sol Medium in the current parent session for orchestration, architecture, integration, review, and acceptance. Prefer Luna xhigh for bounded implementation/specialist work and Luna high for recon/test work. Use `scripts/codex_plus_swarm.py` to compose the smallest useful Luna team from the existing adaptive role classifier: normal mode spawns at most two workers, conservation or critical mode at most one. Never auto-upgrade to Luna max; reserve it for an explicit bounded retry. The native lane records token usage and a credit-equivalent local pacing estimate, but the Codex `/status` output or ChatGPT usage dashboard remains authoritative for actual plan allowance. Read [Codex Plus native lane](references/routing/codex-plus-lane.md) before relying on this path.
...
Both fixed/adaptive API swarms and the Codex-native subscription swarm preserve deterministic role ordering, isolate individual worker failures, avoid credential persistence, and report `proof_status: UNPROVEN` plus `integration_required: true`.
