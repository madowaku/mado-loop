...
Fixed/adaptive API swarms, the Codex-native subscription swarm, and Budget Governor 2.0 preserve deterministic role ordering, isolate individual worker failures, avoid credential persistence, and report `proof_status: UNPROVEN` plus `integration_required: true`.
