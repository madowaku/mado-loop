...
6. **PROVE:** report a schema-v1.1 result using `scripts/common/result.py`, artifact evidence, the achieved P0-P5 level, remaining unknowns, and an AI Creole handoff when work continues across agents. In every final bounded MADO LOOP task receipt, include `skills_used` with canonical registry ids for specialist skills actually opened or invoked across the orchestrator and accepted worker evidence; use an empty list when none were used. OVP mutation receipts are lower-level evidence bundles and do not replace this final provenance field. Recommendations, internal references, worker roles, providers, and ordinary tools do not count as `skills_used`. After the final receipt status is known, if `skills_used` is nonempty and project-local MADO metadata writes are permitted, run `scripts/record_skill_feedback.py` with the stable receipt id, final status, actual repair-cycle count, and only observed token totals when available. Feedback recording is routing telemetry, never proof; failure or absence of the optional feedback store cannot change the task result.
...
Use [Orchestration & Verification Protocol](references/protocol/orchestration-verification.md) whenever a delegated worker will mutate project state. Use `scripts/ovp_runtime.py preflight` or `prepare` before fan-out and isolate every mutation task in the runtime-owned Git worktree. Prefer [OVP Dispatch Adapter](references/protocol/ovp-dispatch.md) plus `scripts/ovp_dispatch.py` to send the generated AI Creole Agent Contract to Codex, Claude, or an explicitly configured local agent. The worker edits/tests/commits only inside its worktree and returns `mado-mutation-handoff/v1`; the orchestrator-side adapter then submits the exact-ID receipt through the authoritative OVP gate. Manual `ovp_runtime.py receipt` remains available for a worker lane not using the adapter. The runtime stores manifests under the Git common directory rather than the tracked checkout, rejects out-of-scope changes, rechecks branch/HEAD identity at review, aborts failed integration, and binds P0-P5 proof to the exact integrated HEAD. Existing read-only swarm runtimes do not become mutation workers merely because they can propose code.
...
- prefer `scripts/ovp_dispatch.py` for provider-neutral mutation dispatch and keep OVP state/receipt authority in the orchestrator-side process;
- require mutation workers to commit their bounded change and end at `REVIEW_READY`, never `DONE`;
- require mutation handoff/receipt check IDs to match the dispatch acceptance IDs exactly;
...
For multi-agent or resumable work, exchange [AI Creole handoffs](references/protocol/ai-creole-handoff.md). For delegated project mutation, also apply the [Orchestration & Verification Protocol](references/protocol/orchestration-verification.md) through `scripts/ovp_runtime.py` and the [OVP Dispatch Adapter](references/protocol/ovp-dispatch.md) through `scripts/ovp_dispatch.py` when a provider process is used. Admit durable observations only under the [FIELD NOTES policy](references/protocol/field-notes-policy.md).
