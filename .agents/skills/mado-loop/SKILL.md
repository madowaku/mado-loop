...
Use [Orchestration & Verification Protocol](references/protocol/orchestration-verification.md) whenever a delegated worker will mutate project state. Run worker-equivalent disposable preflight before fan-out, isolate each mutation assignment in a Git worktree or equivalent workspace, dispatch a bounded AI Creole Agent Contract, require a concrete evidence bundle, review before integration, and run normal P0-P5 proof on the integrated state. Do not reinterpret the existing read-only swarm runtimes as mutation workers unless an implementation explicitly provides these OVP boundaries.
...
- for OVP mutation work, run preflight first and give every mutation worker a separate worktree or equivalent isolated workspace;
...
- require mutation workers to end at `REVIEW_READY`, never `DONE`;
