...
- `ovp_dispatch.py` をfirst-party OVP Dispatch Adapterとして追加。prepared worktreeをCodex / Claude / explicit local agentへprovider-neutralに接続し、AI Creole contractをdispatch、workerにはedit/test/commitと`mado-mutation-handoff/v1`だけを要求し、OVP receipt/state authorityはorchestrator-side adapterに保持
- Dispatch Adapterはparent environmentをwholesale継承せず、secret-looking env pass-throughを明示許可制にし、custom argvとworker completionをpublic metadataからredact。fresh dispatch時のworktree/common-dir/branch/base/clean identityを再検証し、provider crash・invalid handoff・scope escape・missing commitは`WORKING`で停止してexplicit `--resume` recoveryを要求
